...
def Upper_sample(x, is_training=True):
    ...
    net = conv2d_tr(x, [3, 3, 512, 256], [18, 18, 23, 512], [1, 1, 1, 1],
    ...
    net = conv2d_tr(net, [3, 3, 16, 64], [18, 72, 90, 16], [1, 2, 2, 1],
                    is_training=is_training, scope='Upper_sample/net2')
    net = tf.nn.relu(net)
    # shape = (none, 144, 180, 16)

    net = conv2d_tr(net, [3, 3, 3, 16], [18, 72, 90, 128], [1, 1, 1, 1],
        ...
        while step * 18 < max_samples:
            if step % display_step == 0:
                ...
                if loss_g < acc_min:
                    ...
                    if os.path.exists('../log/test_1/' + str(step//display_step)):
                        shutil.rmtree('../log/test_1/' + str(step//display_step))
                    os.mkdir('../log/test_1/' + str(step//display_step))
                    ...
                    for i in range(len(out)):
                        ...
                        img_1.save(
                            '../log/test_1/' + str(step//display_step) +
                            '/' + str(2 * i) + '.jpg')
                        img_2.save(
                            '../log/test_1/' + str(step//display_step) +
                            '/' + str(2 * i + 1) + '.jpg')
                ...
                train_, loss_ = sess.run([train_ga, loss_ga], feed_dict={x: batch_x, y: batch_y})
                del (batch_x)
                del (batch_y)
